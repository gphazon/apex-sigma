...
async def jevvcoin(cmd, message, args):
    ...
    if point_data:
        ...
        response.set_author(name=f'{target.name}\'s Currency2 Data', icon_url=user_avatar(target))
        response.add_field(name='Current Wallet', value=f'```py\n{current_pts} {Currency2}\n```')
    ...
    response.set_footer(text=f'{Currency3} can be earned by being an active member of the server.')
